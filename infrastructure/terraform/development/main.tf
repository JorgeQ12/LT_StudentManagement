...
resource "random_password" "bootstrap_administrator" {
  length           = 24
  special          = true
  override_special = "!#%*+-_=?"
}
...
resource "aws_secretsmanager_secret_version" "application" {
  ...
  secret_string = jsonencode({
    ...
    BootstrapAdministratorEmail    = var.bootstrap_administrator_email
    BootstrapAdministratorPassword = random_password.bootstrap_administrator.result
  })
}
...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      DatabaseInitialization__ApplyMigrations   = tostring(var.apply_database_migrations)
