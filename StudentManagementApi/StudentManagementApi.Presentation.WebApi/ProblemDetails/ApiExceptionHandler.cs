...
internal sealed partial class ApiExceptionHandler(IApiProblemDetailsFactory problemDetailsFactory, ILogger<ApiExceptionHandler> logger) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext httpContext, Exception exception, CancellationToken cancellationToken)
    {
        ...
        var code = concurrencyConflict ? ErrorCode.ConcurrencyConflict : ErrorCode.UnexpectedError;
        var statusCode = concurrencyConflict ? StatusCodes.Status409Conflict : StatusCodes.Status500InternalServerError;
