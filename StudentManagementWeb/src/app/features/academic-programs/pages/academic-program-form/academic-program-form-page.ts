...
@Component({
  selector: 'sm-academic-program-form-page',
  imports: [FormField, LoadingState, Icon, PageHeader, RouterLink],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './academic-program-form-page.html',
  styleUrl: './academic-program-form-page.css',
})
export class AcademicProgramFormPage implements PendingChangesAware {
  ...
  protected async onSubmit(event: Event): Promise<void> {
    ...
    await submit(this.programForm, async () => {
      try {
        if (this.id) {
          await this.facade.updateProgram(this.id, this.model());
        } else {
          await this.facade.createProgram(this.model());
        }
  ...
  private async loadProgram(id: AcademicProgramId): Promise<void> {
    try {
      const program = await this.facade.getProgram(id);
