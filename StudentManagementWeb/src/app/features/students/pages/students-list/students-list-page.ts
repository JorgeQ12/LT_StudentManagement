...
@Component({
  selector: 'sm-students-list-page',
...
export class StudentsListPage {
  private readonly facade = inject(StudentsFacade);
  ...
  protected readonly columns: DataTableColumn<Student>[] = [
    { header: 'Estudiante', render: (s) => `${s.firstName} ${s.lastName}`, kind: 'strong' },
    { header: 'Documento', render: (s) => s.documentNumber, kind: 'muted' },
    { header: 'Correo', render: (s) => s.email, kind: 'muted' },
  ];
    ...
    this.route.queryParamMap
      .pipe(
        tap((params) => {
          ...
          this.status.set((params.get('status') as AccountStatus | null) || '');
        }),
        switchMap(() =>
          this.facade.load$({
  ...
  protected async toggleStatus(item: Student): Promise<void> {
